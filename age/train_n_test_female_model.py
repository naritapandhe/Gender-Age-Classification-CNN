...
def train_and_test():
	...
	for fold in cv_fold_names:
		...
		train_file = load_train_file(train_pickle_file_path_prefix+'female_cv_train_'+fold)
		val_file = load_val_file(train_pickle_file_path_prefix+'female_cv_val_'+fold)
		...
		for i in range(num_steps):
			...
			if (i % 1000 == 0):
				...
				if tacc > past_tacc:
					past_tacc = tacc
					save_path = saver.save(sess, "/media/narita/My Passport/Gender-Age Classification/subject_exclusive_distributed_data/female/saved_model/model.ckpt")
					print("Model saved in file: %s" % save_path)

					pred = np.concatenate(preds)
					np.savetxt('/media/narita/My Passport/Gender-Age Classification/subject_exclusive_distributed_data/female/saved_model/predicted_females_age_prediction.txt',pred,fmt='%.0f') 
